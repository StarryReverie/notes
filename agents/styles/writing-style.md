...
- 同一层级下的并列分类项不加粗，用"标签：描述"格式（如"信道划分：……""随机接入：……"）

### 列表结构

- 使用"标签：内容"格式统领子项，充当无加粗的小标题（如"持续策略：""计算："）
- 同类事物的枚举用嵌套列表，外层用标签概括类别
...
- 极值或特殊条件的表述：把条件与结果写在一行，用"……时有……"句式（如"$G = 0.5$ 时有 $S_{\max} = \dfrac{1}{2e}$"），避免括号补充
