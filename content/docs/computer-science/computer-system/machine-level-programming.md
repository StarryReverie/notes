...
- **过程调用**
    - **栈操作指令**
        - `push? <S>`：把 `<S>` 的值添加到栈顶。
        - `pop? <D>`：弹出栈顶并把值赋值给 `<D>`。
    - **x86_64 System V 调用约定**
        - 栈底位于高地址，栈顶位于低地址。
        - 每次函数调用产生一个栈帧，返回时栈帧销毁。
            - 一个栈帧中，从栈底到栈顶依次保存额外函数参数（如果有）、函数返回地址、原 `%rbp`（如果未启用 FPO）、保护寄存器、局部变量。
            - 函数参数优先保存在寄存器 `%rdi`、`%rsi`、`%rdx`、`%rcx`、`%r8`、`%r9`，其次保存在栈上。
            - 存在额外函数参数时，完成调用后， caller 清理参数并恢复 `%rsp` 到开始调用前的状态。
            - 默认情况下，启用 FPO，不在栈帧中保存原 `%rbp`。
                - 函数参数和局部变量都通过 `-?(%rsp)` 来引用。
            - 未启用 FPO 时，callee 在开始时 `push %rbp`，再把 `%rsp` 复制给 `%rbp`，用 `%rbp` 作为栈帧基址。
                - 额外函数参数用 `16(%rbp)`、`24(%rbp)` 依次表示，此时 `(%rbp)` 是保存的原 `%rbp`，`8(%rbp)` 是返回地址。
                - 局部变量用 `-?(%rbp)`、 `-?-8(%rbp)` 引用，其中 `?` 是由于保存的寄存器而产生的。
        - 寄存器保护规则：
            - caller 保护寄存器：`%rax`、`%rcx`、`%rdx`、`%rdi`、`%rsi`、`%r8`~`%r11`，被调函数参数和返回值包括在其中。
            - callee 保护寄存器：`%rbx`、`%rbp`、`%rsp`、`%r12`~`%r15`，需要在返回时恢复这些寄存器为调用前的值。
        - 执行 `call` 指令时，要求 `%rsp` 是 16 字节对齐，如果不对齐，则需要额外添加空位。
        - 当前执行的函数存在 red zone，`%rsp` 下 128 字节可以在不修改 `%rsp` 的情况下使用。
            - 如果函数调用另外一个函数， caller 的 red zone 失效，会被 callee 的栈帧覆盖。
            - 中断发生、[信号](/docs/computer-science/computer-system/exceptional-control-flow#qnfiug)接收时，不保证 red zone 的有效性。
    - **过程调用指令**
        - `call <L>`：调用函数 `<L>`，`<L>` 是函数起始代码地址的标号，会自动填写返回地址并修改 `%rsp`。
        - `call *<D>`：`*` 是固定格式，目标地址从 `<D>` 中读取。
        - `retq`：读取返回地址，跳转回 caller 函数，自动修改 `%rsp`。
- **复合数据结构**
    - **数组**
        - **一维数组**
